...
def main():
    ...
    for img in images:
        print("{} selected for processing!".format(img))
        # read image in grayscale
        image = cv2.imread(path + img + ext)
        w, h, _ = image.shape
        ...
        while True:
            ...
            gray, th, edged, cnt = getFiltred2(image.copy(), blur, sigma, thkernel, thparam, dilate_iter, erode_iter)
            ...
            if cv2.waitKey(1) & 0xFF == ord('s'):
                np.savez(pathDst + img, cnt)
                print('Contour {} saved!\nNext image!!'.format(img))
                break
